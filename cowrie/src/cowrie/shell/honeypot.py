...
class HoneyPotShell:
    ...
    def lineReceived(self, line: str) -> None:
        ...
        # qrassh code to decide the action for the command and write to policy.log
        session_id = str(getattr(self.protocol, "sessionno", "") or "")
        if cmd:
            decide_action(cmd, session_id=session_id)
            if cmd == "exit":
                reset_session()
